...
class Solution {
  public:
    void correctBST( struct Node* root )
    {
        ...
        findFault(root, prev, node1, node2);

        swap(node1->data, node2->data);
    }
    
    void findFault(Node* root, Node* &prev, Node* &node1, Node* &node2) {
        if (!root) {
            return;
        }
        
        findFault(root->left, prev, node1, node2);
        ...
        findFault(root->right, prev, node1, node2);
    }
};
